...
class can_transaction extends uvm_sequence_item;
	...
	rand bit [28:0] id; // valid bits depending upon the can_fmt
	rand bit [3:0] dlc; // Classic can : 0...8
	rand bit f_type; // DATA,REMOTE,OVERLOAD,ERROR
	rand byte unsigned data[]; // Payload dynamic array 
	
	// ----------Error injection ( driver will implement physically )---
	rand bit inj_crc_error;
	rand bit inj_stuff_error;
	rand bit inj_form_error;
	rand bit inj_ack_error;
	
	// ------- Observability / debug ( monitor can fill these in)
	time t_start;
	time t_end;
	bit [14:0] crc_obs;
	...
	constraint c_payload_vs_type {(f_type == `CAN_DATA_FRAME)  -> (data.size() == dlc);(f_type != `CAN_DATA_FRAME)  -> (data.size() == 0);}
	constraint c_error_overload_sanity { (f_type inside {`CAN_ERROR_FRAME, `CAN_OVERLOAD_FRAME}) -> (dlc == 0 && can_fmt == `CAN_ID_STD && id == '0);}
	...
	// ---------- Optional ( convert2string for Logs ) -------------------------
	...
	// -------- optional: Custom do_print ( more readable than default) 
