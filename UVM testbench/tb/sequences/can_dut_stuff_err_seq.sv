`ifndef CAN_DUT_STUFF_ERR_SEQ_SV
`define CAN_DUT_STUFF_ERR_SEQ_SV

class can_dut_stuff_err_seq extends uvm_sequence #(can_transaction);
  `uvm_object_utils(can_dut_stuff_err_seq)

  virtual can_if #(.NUM_TB_NODES(3)) vif;

  // PeliCAN register addresses
  localparam byte MOD   = 8'h00;
  localparam byte CMR   = 8'h01; // not used here, but kept for consistency
  ...
  function new(string name = "can_dut_stuff_err_seq");
    super.new(name);
  endfunction
  ...
  task body();
    ...
    `uvm_info("STUFF_ERR", "Starting STUFF error injection test (DUT in NORMAL mode)", UVM_LOW)
    ...
    // -- Send DATA frame with STUFF error ----------------------
    ...
    assert(tx_tr.randomize() with {
      ...
      inj_form_error  == 0;
      inj_ack_error   == 0;
    }) else `uvm_fatal("STUFF_ERR", "TX randomization failed")
    finish_item(tx_tr);
    ...
    if (tx_tr.ack_seen)
      `uvm_error("STUFF_ERR", "FAIL: DUT ACKed a STUFF-corrupted frame — should not happen")
    else
      `uvm_info("STUFF_ERR", "PASS: DUT did not ACK the STUFF-corrupted frame", UVM_LOW)

    // Give time for error handling and possible ECC latch
    wait_irq_any(200us, irq_seen);
    if (!irq_seen)
      `uvm_warning("STUFF_ERR", "irq_on did not toggle (not fatal); reading ECC/IR anyway.")
    ...
    if (sr_val[0])
      ...
      `uvm_info("STUFF_ERR", "PASS: DUT RX buffer empty — corrupt frame rejected", UVM_LOW)

    `uvm_info("STUFF_ERR", "===== STUFF ERROR INJECTION TEST COMPLETE =====", UVM_LOW)
  endtask
