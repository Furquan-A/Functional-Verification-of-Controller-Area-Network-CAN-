`ifndef CAN_DUT_FORM_ERR_TEST_SV
`define CAN_DUT_FORM_ERR_TEST_SV


class can_dut_form_err_test extends uvm_test;
  `uvm_component_utils(can_dut_form_err_test)

  virtual can_if     vif;
  can_env            m_env;
  can_env_config     env_cfg;

  function new(string name="can_dut_form_err_test", uvm_component parent=null);
  ...
  function void build_phase(uvm_phase phase);
    ...
    if (!uvm_config_db#(virtual can_if)::get(this, "", "vif", vif))
      `uvm_fatal("FORM_ERR_TEST", "cannot get vif")
  ...
  task run_phase(uvm_phase phase);
    can_dut_form_err_seq form_seq;

    phase.raise_objection(this);

    form_seq     = can_dut_form_err_seq::type_id::create("form_seq");
    form_seq.vif = vif;

    form_seq.start(m_env.c_agent[0].seqrh);
