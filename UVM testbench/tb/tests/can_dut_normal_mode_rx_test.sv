`ifndef CAN_DUT_NORMAL_MODE_RX_TEST_SV
`define CAN_DUT_NORMAL_MODE_RX_TEST_SV

class can_dut_normal_mode_rx_test extends uvm_test;
  `uvm_component_utils(can_dut_normal_mode_rx_test)

  virtual can_if     vif;
  can_env            m_env;
  can_env_config     env_cfg;

  function new(string name="can_dut_normal_mode_rx_test", uvm_component parent=null);
  ...
  function void build_phase(uvm_phase phase);
    ...
    if (!uvm_config_db#(virtual can_if)::get(this, "", "vif", vif))
      `uvm_fatal("DUT_NORM_RX_TEST", "cannot get vif")

    // Env config: 1 active agent (transmitter)
    ...
    // Agent is transmitter here. ACK enable doesn't matter for TX,
    // but keep defaults.
  ...
  task run_phase(uvm_phase phase);
    can_dut_normal_mode_rx_seq seq;
  
    phase.raise_objection(this);
  
    seq     = can_dut_normal_mode_rx_seq::type_id::create("seq");
    seq.vif = vif;
  
    // Start on agent sequencer (because seq uses start_item/finish_item)
