`ifndef CAN_DUT_SELF_TEST_MODE_TEST_SV
`define CAN_DUT_SELF_TEST_MODE_TEST_SV

`include "uvm_macros.svh"
import uvm_pkg::*;
import can_pkg::*;

class can_dut_self_test_mode_test extends uvm_test;
  `uvm_component_utils(can_dut_self_test_mode_test)

  virtual can_if     vif;
  can_env            m_env;
  can_env_config     env_cfg;

  function new(string name="can_dut_self_test_mode_test", uvm_component parent=null);
  ...
  function void build_phase(uvm_phase phase);
    ...
    if (!uvm_config_db#(virtual can_if)::get(this, "", "vif", vif))
      `uvm_fatal("DUT_STM_TEST", "cannot get vif")

    env_cfg     = can_env_config::type_id::create("env_cfg");
    env_cfg.vif = vif;

    // 1 node present just to monitor the bus (no ACK)
    env_cfg.resize(1, 0);
    ...
    // Make the agent passive so it won't drive/ACK anything
  ...
  task run_phase(uvm_phase phase);
    can_dut_self_test_mode_seq seq;

    phase.raise_objection(this);

    seq     = can_dut_self_test_mode_seq::type_id::create("seq");
    seq.vif = vif;

    // reg-driven sequence (no start_item), so start on null is correct
    seq.start(null);
