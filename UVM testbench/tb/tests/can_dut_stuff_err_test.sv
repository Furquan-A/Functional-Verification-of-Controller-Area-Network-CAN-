`ifndef CAN_DUT_STUFF_ERR_TEST_SV
`define CAN_DUT_STUFF_ERR_TEST_SV



class can_dut_stuff_err_test extends uvm_test;
  `uvm_component_utils(can_dut_stuff_err_test)

  virtual can_if     vif;
  can_env            m_env;
  can_env_config     env_cfg;

  function new(string name="can_dut_stuff_err_test", uvm_component parent=null);
  ...
  task run_phase(uvm_phase phase);
    can_dut_stuff_err_seq stuff_seq;

    phase.raise_objection(this);

    stuff_seq     = can_dut_stuff_err_seq::type_id::create("stuff_seq");
    stuff_seq.vif = vif;

    stuff_seq.start(m_env.c_agent[0].seqrh);
